...
for (const flights of flightsArr) {
  const splittedFlight = flights.split(';');

  console.log(
    `${splittedFlight[0].includes('Delayed') ? '🔴' : ''} ${splittedFlight[0]
      .slice(1)
      .replace('_', ' ')} from ${splittedFlight[1]
      .slice(0, 3)
      .toUpperCase()} to ${splittedFlight[2]
      .slice(0, 3)
      .toUpperCase()} (${splittedFlight[3].replace(':', 'h')})`
  );
}
